...
```
Power On
  → CSME ROM boots and authenticates CSME FW from SPI (ME region)
  → CSME enforces fuse-based Boot Guard policy
  ...
  → ACM verifies initial boot block (IBB: Stage1A) using KM/BPM chain
...
```
CSME ROM executes
  → Loads CSME firmware from SPI flash ME region (IFWI)
  → Verifies CSME FW signature
  → Reads Boot Guard policy fuses (FPF)
  → Validates/authorizes boot policy structures (KM/BPM)
  → Releases CPU reset
```
