...
graph TB
    ...
    D --> E[CSME Prepares Platform / PMC Patch]
    E --> F[CSME Releases CPU from Reset]
    F --> G[CPU Reset Vector 0xFFFFFFF0]
    G --> H[CPU Loads Microcode via FIT]
    H --> I[CPU Loads ACM via FIT]
    I --> J[CPU Verifies ACM Signature Intel Key]
    J --> K[ACM Validates KM / BPM Chain]
    K --> L[ACM Verifies IBB Stage 1A Hash]
    L --> M[Control Transfers to Stage 1A]
```

**Critical Corrections:**
1. ❌ **Incorrect**: "CSME is the authoritative manifest validator"
   - ✅ **Correct**: CSME performs preparatory gating; **ACM** is the authoritative cryptographic auditor that re-reads fuses and validates KM/BPM/IBB.
2. ❌ **Incorrect**: "CPU queries CSME for ACM validation"
   - ✅ **Correct**: CPU verifies ACM signature using Intel public key fused in CPU hardware
3. ❌ **Incorrect**: "CSME provides OEM key hash to ACM at runtime"
   - ✅ **Correct**: OEM key hash is fused in FPF and read directly by ACM
...
3. Performs platform initialization as the "Starter Motor" (Lines 705, 483).
4. Reads Boot Guard configuration from Field Programmable Fuses (FPF).
5. **Prepares** platform security by gating the CPU reset until fuses are read.
6. Makes the Key Manifest (KM) and Boot Policy Manifest (BPM) available for ACM.
7. Controls reset gating - releases CPU once platform ground is established.

**What CSME Does NOT Do:**
- ❌ Does not cryptographically verify ACM signature (CPU does this)
- ❌ Does not verify IBB hash (ACM does this)
- ❌ Does not participate in runtime co-verification with ACM
...
#### 1. CSME Architecture
❌ **Incorrect**: "CSME is a separate i486-based processor"  
✅ **Correct**: CSME 18.x uses an **ARC-based microcontroller** (32-bit, custom instruction set)
...
❌ **Incorrect**: "CSME and ACM jointly verify IBB hash"  
✅ **Correct**: CSME validates manifests (KM/BPM) **before CPU release**; ACM independently verifies IBB **after CPU release**
...
❌ **Incorrect**: "ACM contacts CSME to get OEM key hash at runtime"  
✅ **Correct**: OEM key hash is fused in FPF (Field Programmable Fuses) and read directly by ACM
...
❌ **Incorrect**: "CPU queries CSME for ACM validation"  
✅ **Correct**: CPU verifies ACM signature using Intel public key **fused in CPU hardware**
...
❌ **Incorrect**: "CSME enforces additional DBI (Disable BSP INIT) protections"  
✅ **Correct**: DBI is **not documented** in official Intel Arrow Lake specifications
...
#### 6. Manifest Location
❌ **Incorrect**: "ACM reads KM/BPM from FIT entries"  
✅ **Correct**: KM and BPM are signed structures stored in flash; FIT contains **pointers to ACM and microcode**, not manifests
...
✅ **Verified**: Slim Bootloader uses the IDTR register to store the `LdrGlobal` pointer during early boot stages
...
✅ **Verified**: Arrow Lake uses FSP 3.0 with the standard T/M/S split
...
#### CSME 18.x Features
✅ **Verified**: CSME 18.x includes enhanced security features from version 16 (Alder Lake)
