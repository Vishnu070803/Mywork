...
3. **Intel® CSME Security Technical White Paper (October 2022)**: [Official CSME Security Reference]

---

## Key Confirmations from Official Sources

### 1. The "Starter Motor" Role (CSME Source)
The October 2022 White Paper clarifies that CSME is responsible for the platform's silicon-level inception:
*   **Initialization**: *"Intel® CSME role is **essential in the platform silicon initialization and platform boot**. It is responsible for: **Base initialization of PCH, including configuration of clocks and GPIO**..." (Lines 273-276)*.
*   **PMC Dependency**: CSME must apply a firmware patch to the Power Management Controller (PMC) before the CPU starts. *"A system cannot boot without a proper, PMC patch." (Line 705)*.

### 2. Chain of Trust Origin (Intel Source)
...
### 1. Terminology: BPM vs KM vs IBB

**My previous documentation:** Used "IBBM" (from 2020 Medium article)  
**Official Intel documentation:** Uses **KM** (Key Manifest) and **BPM** (Boot Policy Manifest).

**Clarification:**
- **KM**: Authorizes the OEM public key for signing the BPM.
- **BPM**: Contains the authoritative hashes for the **IBB** (Initial Boot Block) segments.
- **BPM is the correct official term** for the policy structure that defines what to verify.
...
1. Intel Developer Zone - [Introduction to Key Usage in Integrated Firmware Images](https://www.intel.com/content/www/us/en/developer/articles/technical/software-security-guidance/resources/key-usage-in-integrated-firmware-images.html)
2. TianoCore - [Understanding UEFI Secure Boot Chain - Intel Boot Guard](https://tianocore-docs.github.io/Understanding_UEFI_Secure_Boot_Chain/draft/secure_boot_chain_in_uefi/intel_boot_guard.html)
3. Intel® CSME Security Technical White Paper (October 2022)
4. Dell EMC - [Cyber-Resiliency in Chipset and BIOS](https://downloads.dell.com/solutions/servers-solution-resources/Direct%20from%20Development%20-%20Cyber-Resiliency%20In%20Chipset%20and%20BIOS.pdf)
