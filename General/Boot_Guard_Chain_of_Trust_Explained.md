...
graph TD
    ...
    B --> C[CSME Firmware]
    C --> D[Boot Guard Policy FPF Fuses]
    ...
    K --> L[KM Validated]
    L --> M[BPM Validated]
    M --> N[IBB Hash Verified]
    N --> O[Stage1A Executes]
...
|-----------|--------------|-------------------|-------------------|
| **CSME ROM** | Immutable ROM in PCH | Hardware (cannot be changed) | CSME Firmware |
| **CSME Firmware** | RSA signature in flash | CSME ROM (Intel public key) | Boot Guard Manifests |
| **FPF Fuses** | Hardware fuses (OTP) | Physically programmed at factory | Nothing (stores trust anchor hash) |
| **CPU Microcode** | RSA signature | CPU built-in Intel key | Nothing (enables CPU features) |
| **ACM** | RSA signature | CPU built-in Intel key | KM/BPM/IBB chain |
| **KM (Key Manifest)** | RSA signature | Hash in FPF fuses | BPM signature |
| **BPM (Boot Policy)** | RSA signature | Public key from KM | IBB hash |
| **IBB (Stage1A)** | SHA256/384 hash | Hash list in BPM | Next stages (SBL) |
...
### Stage 12: ACM Verifies IBB (Stage1A) Hash

**Question:** How does ACM verify IBB? Again, how?

**Answer:** ACM reads the **IBB hash(es) from the validated BPM** and compares them with the **actual hash** of firmware in flash

#### IBB Verification Process:

```
ACM has access to (CSME provided in special memory region):
  ✅ Validated Key Manifest (KM)
  ✅ Validated Boot Policy Manifest (BPM)
  ✅ Boot Guard policy from FPF fuses
...
**Trust Chain:** BPM.IBB_Digest (already validated via KM/fuse chain) → Actual IBB Code (calculated hash) → Comparison

**Key Point:** ACM doesn't trust anything - it **calculates the actual hash** and compares with the **pre-validated expected hash** from BPM.

---

### Stage 13: Control Transfers to Stage1A
