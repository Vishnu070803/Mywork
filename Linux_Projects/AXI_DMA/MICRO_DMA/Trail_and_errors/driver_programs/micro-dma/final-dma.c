...
static ssize_t attr_store(struct platform_device *pdev, struct device_attribute *attr, const char *buf, size_t count)
{   
	unsigned long value;
	int ret;
	struct custom_dma_device *ddev = platform_get_drvdata(pdev);
	pr_info("this is sample base address 0x%lx", ddev->base_address);
    if (strcmp(attr->attr.name, "srcaddr") == 0){
...
static int custom_dma_probe(struct platform_device *pdev){
	...
	ddev->dma_size = res->size;
  	pr_info("my_driver: AXI DMA size = 0x%lxa\n", ddev->dma_size);
  	pr_info("my_driver: AXI DMA physical address start = 0x%lxa\n", ddev->base_address);
	...
	/* getting address width from device tree */
