...
static int axi_dma_transfer(size_t length) {
   int i;
    ...
    for (i = 0; i < 10; i++) { // Print first 10 values
        pr_info(DRIVER_NAME ": dest_buffer[%d] = 0x%08x\n", i, dest_buffer[i]);
    }
...
static int xilinx_dma_probe(struct platform_device *pdev)
{
    ...
    src_buffer = dma_alloc_coherent(&pdev->dev, PAGE_SIZE, &src_dma_addr, GFP_KERNEL);
    dest_buffer = dma_alloc_coherent(&pdev->dev, PAGE_SIZE, &dest_dma_addr, GFP_KERNEL);
...
static int xilinx_dma_remove(struct platform_device *pdev)
{
    ...
    dma_free_coherent(&pdev->dev, PAGE_SIZE, src_buffer, src_dma_addr);
    dma_free_coherent(&pdev->dev, PAGE_SIZE, dest_buffer, dest_dma_addr);
